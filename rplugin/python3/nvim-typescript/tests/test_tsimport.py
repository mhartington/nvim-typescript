...
def generateCandidatesTest(currentFile, symbol, expected, tsserverOutput):
    def test(self):
        ...
        candidates = tsimport.getImportCandidates(client, currentFile,symbol)
        self.assertEqual(candidates, expected)
    return test

# Generate tsimport.getImportCandidates tests
...
# Generate tsimport.getRelativeImportPath tests
...
def generateImportPathTests(currentFile, symbolFile, expected):
    def test(self):
        self.assertEqual(tsimport.getRelativeImportPath(currentFile, symbolFile), expected)
    return test
